...
@Slf4j
@NoArgsConstructor
public class InseeLineRecordReader extends RecordReader<InseeLongWritable, InseeText> {
    ...
    private InseeLongWritable key;
    private InseeText value;
    private boolean isCompressedInput;
    private Decompressor decompressor;
    private byte[] recordDelimiterBytes;


    public InseeLineRecordReader(byte[] recordDelimiter) {
    ...
    public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException {
        ...
        if (this.start != 0L) {
            this.start += (long)this.in.readLine(new InseeText(), 0, this.maxBytesToConsume(this.start));
        }
    ...
    public boolean nextKeyValue() throws IOException {
        if (this.key == null) {
            this.key = new InseeLongWritable();
        }

        this.key.set(this.pos);
        if (this.value == null) {
            this.value = new InseeText();
        }
    ...
    public InseeLongWritable getCurrentKey() {
        return this.key;
    }

    public InseeText getCurrentValue() {
        return this.value;
    }
